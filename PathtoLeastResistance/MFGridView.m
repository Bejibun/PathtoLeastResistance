...
- (void)updateSubviews
{
    ...
    for (NSUInteger column = left; column <= right; ++column) {
        for (NSUInteger row = top; row <= bottom ; ++row) {
            ...
            MFGridViewIndex *index =[[MFGridViewIndex alloc] init];
            index.column = column;
            index.row = row;
...
//get cell value
- (int)getCellValue:(MFGridViewIndex *)index
{
    NSArray *cells = self.subviews;
    for (MFGridViewCell *cell in cells) {
        ...
        if (cell.index != nil && cell.index.row == index.row && cell.index.column == index.column) {
            ...
            MFGridViewCell *currentCell = [self subviewForIndex:index];
            NSLog(@"Current Value: %d", (int)currentCell.input.text.integerValue);
            return (int)currentCell.input.text.integerValue;
        }
...
- (void)setRandom:(MFGridViewCell *)currentCell
{
    [currentCell.input setText:[NSString stringWithFormat:@"%d",[self randomInRangeLo:MIN_RANDOM toHi:MAX_RANDOM]]];
}

- (int)randomInRangeLo:(int)loBound toHi:(int)hiBound
{
    int range = hiBound - loBound + 1;
